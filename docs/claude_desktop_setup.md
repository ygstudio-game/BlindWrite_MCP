...
In the Claude Desktop App, skills are associated directly with your Anthropic account in the cloud rather than read from a local folder. You have two instant options:
- **Option A: Upload Skill (1 Click)**:
  1. In Claude Desktop, go to **Customize** > **Skills**.
  2. Click the **`+`** button and select **"Upload a skill"**.
  3. Select the pre-packaged ZIP archive generated by the setup script:
     ```
     skills/writing-orchestrator.zip
     ```
  4. Ensure the `writing-orchestrator` skill is toggled **ON**.
- **Option B: Native MCP Prompt (Zero Upload)**:
  BlindWrite MCP automatically registers an official MCP Prompt named `writing-orchestrator`. You can invoke it immediately in Claude Desktop chat with `/writing-orchestrator`.
